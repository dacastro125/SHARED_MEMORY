...
int main() {
    system("clear");
    char archivo[50] = "/home/felipe/Escritorio/Tallersharememory/datos";
    ...
    *turno = 0;
        ...
        if (!fork()) {
            for (int j = 0; j < num_iteraciones; j++) {
                if (i == 0) {
                    ...
                    printf("Soy el proceso [%d] y realizo la suma en la iteración %d\n",getpid(), j);
                    float suma = 0;
                    for (int d = 0; d < c; d++) suma += vector[d];
                    result[0] = suma;
                    *turno = 1;
                } else if (i == 1) {
                    while (*turno != 1) usleep(100);
                    printf("Soy el proceso [%d] y saco el promedio en la iteración %d\n", getpid(), j);
    ...
    if (i == num_procesos) {
        ...
        for (int a = 0; a < c; a++) fscanf(file, "%d", &vector[a]);
        fclose(file);
        *turno = 0;
        for (int b = 0; b < num_procesos; b++) wait(NULL);
